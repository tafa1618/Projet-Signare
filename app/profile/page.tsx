...
export default function ProfilPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white pb-24 w-full max-w-[100vw] overflow-x-hidden">
      {/* Top Bar */}
      <div className="sticky top-0 z-40 bg-[#0A0A0A]/95 backdrop-blur-xl border-b border-[#D4AF37]/10">
      ...
      >
        ...
        <section className="relative">
          ...
          <div className="relative w-full h-48 sm:h-56 bg-gradient-to-br from-[#D4AF37]/20 to-[#0A0A0A] overflow-hidden">
            ...
            {isOwnProfile && (
              <div className="absolute top-3 right-3 flex gap-2">
                {/* Mobile Settings Button (visible small screens) */}
                <button
                  onClick={() => router.push('/settings')}
                  className="lg:hidden bg-[#0A0A0A]/60 backdrop-blur-md border border-[#D4AF37]/30 text-[#D4AF37] p-2 rounded-lg hover:bg-[#D4AF37]/20 transition-all shadow-lg"
                  aria-label="Paramètres"
                >
                ...
                {/* Camera Button */}
                <button
                  className="bg-[#0A0A0A]/60 backdrop-blur-md border border-[#D4AF37]/30 text-[#D4AF37] p-2 rounded-lg hover:bg-[#D4AF37]/20 transition-all shadow-lg"
          ...
          {/* Stats Client (Pivot: Focus Commandes & Mesures) */}
          <div className="px-4 sm:px-6 mt-6 flex items-center justify-center gap-8 border-t border-[#D4AF37]/10 pt-6">
            <div className="flex flex-col items-center group cursor-pointer hover:bg-white/5 p-2 rounded-xl transition-colors">
              <Package className="w-6 h-6 text-[#D4AF37] mb-1 group-hover:scale-110 transition-transform" />
              <span className="text-xl sm:text-2xl font-serif text-white font-bold">3</span>
              <span className="text-[10px] text-white/50 uppercase tracking-[0.2em] font-black">Commandes</span>
            </div>
            <div
              onClick={() => router.push('/atelier/measure/agent')}
              className="flex flex-col items-center group cursor-pointer hover:bg-white/5 p-2 rounded-xl transition-colors"
            >
              <Ruler className="w-6 h-6 text-[#D4AF37] mb-1 group-hover:scale-110 transition-transform" />
              <span className="text-xl sm:text-2xl font-serif text-white font-bold">1</span>
              <span className="text-[10px] text-white/50 uppercase tracking-[0.2em] font-black">Profil Mesure</span>
            </div>
            <div className="flex flex-col items-center group cursor-pointer hover:bg-white/5 p-2 rounded-xl transition-colors">
              <Heart className="w-6 h-6 text-[#D4AF37] mb-1 group-hover:scale-110 transition-transform" />
              <span className="text-xl sm:text-2xl font-serif text-white font-bold">12</span>
              <span className="text-[10px] text-white/50 uppercase tracking-[0.2em] font-black">Wishlist</span>
            </div>
          </div>

          {/* Action Buttons */}
          <div className="px-4 sm:px-6 mt-8 flex flex-col gap-3">
            <button className="w-full bg-[#D4AF37] text-black font-bold py-3 rounded-xl uppercase tracking-widest text-sm hover:bg-white transition-colors flex items-center justify-center gap-2">
            ...
            {mode === 'client' && (
              <button
                onClick={() => router.push('/atelier/measure/agent')}
                className="w-full bg-white/5 border border-white/10 text-white font-medium py-3 rounded-xl uppercase tracking-widest text-xs hover:bg-white/10 transition-colors flex items-center justify-center gap-2"
              >
                <Ruler className="w-4 h-4" />
                Mettre à jour mesures (IA)
              </button>
            )}
            <button className="w-full bg-white/5 border border-white/10 text-white font-medium py-3 rounded-xl uppercase tracking-widest text-xs hover:bg-white/10 transition-colors flex items-center justify-center gap-2">
              <Settings className="w-4 h-4" />
        ...
        <div className="px-4 sm:px-6 mt-8 space-y-6">
          ...
          {mode === 'client' ? (
            <>
              {/* Mes Commandes Récentes (Nouveau) */}
              <section className="space-y-4">
                <div className="flex items-center justify-between">
                  <h2 className="text-sm font-serif text-[#D4AF37] tracking-[0.18em] uppercase">Commandes en cours</h2>
                  <span className="text-xs text-white/50">Voir tout</span>
                </div>

                {/* Mock Order Card */}
                <div className="bg-[#111] border border-white/10 rounded-xl p-4 flex gap-4">
                  <div className="w-16 h-20 bg-white/5 rounded-lg relative overflow-hidden">
                    <Image src="https://images.unsplash.com/photo-1548142340-97e3cb7d206c?w=200" alt="Tenue" fill className="object-cover" />
                  </div>
                  <div className="flex-1">
                    <div className="flex justify-between items-start">
                      <div>
                        <h3 className="text-white font-bold text-sm">Boubou Tabaski Luxe</h3>
                        <p className="text-xs text-white/50 mt-1">Atelier Moussa</p>
                      </div>
                      <span className="bg-[#D4AF37]/20 text-[#D4AF37] text-[10px] px-2 py-1 rounded-full font-bold uppercase">En cours</span>
                    </div>
                    <div className="mt-3 flex items-center justify-between">
                      <span className="text-[#D4AF37] font-serif font-bold text-sm">85 000 FCFA</span>
                      <button className="text-xs text-white/60 underline decoration-white/30">Suivre</button>
                    </div>
                  </div>
                </div>
              </section>
