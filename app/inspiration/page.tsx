    ...
    <div className="min-h-screen bg-[#0A0A0A] text-white pb-24">
      <header className="sticky top-0 z-40 bg-[#0A0A0A]/90 backdrop-blur-xl border-b border-[#D4AF37]/20 px-6 py-4">
        <div className="max-w-2xl mx-auto flex items-center gap-3">
          <div className="bg-[#D4AF37] p-2 rounded-lg shadow-[0_0_16px_rgba(212,175,55,0.35)]">
            <Wand2 className="w-5 h-5 text-[#0A0A0A]" />
          </div>
          <div>
            <h1 className="text-lg font-serif text-[#D4AF37] tracking-[0.2em] uppercase">Inspiration IA</h1>
            <p className="text-[10px] uppercase tracking-[0.22em] text-white/40 font-black">Essayage + Moodboard</p>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-2xl mx-auto px-4 pt-6 space-y-8">
        ...
        <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-3 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`px-3 py-2 rounded-full text-[10px] font-black uppercase tracking-[0.18em] border transition-all ${
                mode === 'client'
        ...
        {mode === 'client' && (
        <section className="bg-white/[0.03] border border-white/10 rounded-2xl p-4 space-y-4">
          <div className="flex items-center gap-2">
            <Camera size={16} className="text-[#D4AF37]" />
            <h2 className="text-sm font-serif text-[#D4AF37] tracking-[0.18em] uppercase">Essayage IA</h2>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            >
              <span className="text-[10px] uppercase tracking-[0.22em] text-white/40 font-black">Photo utilisateur (drag & drop)</span>
              <div className="mt-2 w-full bg-[#0A0A0A] border border-white/10 rounded-xl px-4 py-6 text-center flex flex-col items-center justify-center gap-2 hover:border-[#D4AF37]/40 transition-colors">
                {userImage ? (
                  <div className="relative w-full aspect-[3/4] max-w-[220px] mx-auto overflow-hidden rounded-lg border border-[#D4AF37]/25">
                    <Image src={userImage} alt="Utilisateur" fill className="object-cover" />
                  </div>
                ) : (
                  <>
                    <Camera size={18} className="text-[#D4AF37]" />
                    <p className="text-sm text-white/60">Glissez votre portrait ou cliquez pour choisir</p>
                  </>
                )}
              </div>
              <input
              ...
              />
              <p className="mt-1 text-[11px] text-white/35">Portrait clair, frontal.</p>
            </label>
            ...
            <label
              ...
              className="block cursor-pointer"
            >
              <span className="text-[10px] uppercase tracking-[0.22em] text-white/40 font-black">Photo du modèle (drag & drop)</span>
              <div className="mt-2 w-full bg-[#0A0A0A] border border-white/10 rounded-xl px-4 py-6 text-center flex flex-col items-center justify-center gap-2 hover:border-[#D4AF37]/40 transition-colors">
                {modelImage ? (
                  <div className="relative w-full aspect-[3/4] max-w-[220px] mx-auto overflow-hidden rounded-lg border border-[#D4AF37]/25">
                    <Image src={modelImage} alt="Modèle" fill className="object-cover" />
                  </div>
                ) : (
                  <>
                    <Shirt size={18} className="text-[#D4AF37]" />
                    <p className="text-sm text-white/60">Glissez la tenue de référence ou cliquez</p>
                  </>
                )}
              </div>
              <input
              ...
              />
              <p className="mt-1 text-[11px] text-white/35">Tenue à tester sur vous.</p>
            </label>
          </div>

          <label className="block">
            <span className="text-[10px] uppercase tracking-[0.22em] text-white/40 font-black">Prompt (style, matière, occasion)</span>
            <textarea
              value={prompt}
              onChange={(e) => setPrompt(e.target.value)}
              rows={3}
              className="mt-2 w-full bg-white/[0.03] border border-white/10 rounded-xl px-4 py-3 outline-none focus:border-[#D4AF37]/40 text-sm leading-relaxed placeholder:text-white/25"
          ...
          <motion.button
            ...
            className="w-full bg-[#D4AF37] text-[#0A0A0A] py-3 rounded-xl font-black uppercase tracking-[0.2em] text-[11px] shadow-[0_0_18px_rgba(212,175,55,0.35)] disabled:opacity-40 disabled:grayscale"
          >
            Simuler sur moi
          </motion.button>
          ...
          {tryOnResults.length > 0 && (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {tryOnResults.map((r) => (
                <div key={r.id} className="bg-[#0A0A0A] border border-white/10 rounded-xl overflow-hidden">
                  <div className="relative aspect-[3/4] bg-neutral-900">
                    <Image src={r.outputImage} alt="Résultat IA" fill className="object-cover" />
                    <div className="absolute inset-0 bg-gradient-to-t from-[#0A0A0A]/60 to-transparent" />
                    <div className="absolute bottom-2 left-2 text-[10px] uppercase tracking-[0.22em] text-[#D4AF37] font-black flex items-center gap-1">
                      <Sparkle size={12} />
                      IA
                    </div>
                  </div>
                  <div className="px-3 py-3 space-y-1">
                    <p className="text-xs text-white/80 line-clamp-2">{r.prompt}</p>
                    <p className="text-[10px] text-white/40 uppercase tracking-[0.2em]">Simulation</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        {mode === 'tailleur' && (
        <section className="bg-[#0A0A0A] border border-[#D4AF37]/20 rounded-2xl p-4 space-y-4">
          <div className="flex items-center gap-2">
            <Palette size={16} className="text-[#D4AF37]" />
            <h2 className="text-sm font-serif text-[#D4AF37] tracking-[0.18em] uppercase">Inspiration Tailleur</h2>
          </div>
          ...
          <label
            ...
            className="block cursor-pointer"
          >
            <span className="text-[10px] uppercase tracking-[0.22em] text-white/40 font-black">Moodboard (jusqu'à 4 photos)</span>
            <div className="mt-2 w-full bg-white/[0.02] border border-white/10 rounded-xl px-4 py-4 text-center flex flex-col items-center justify-center gap-2 hover:border-[#D4AF37]/30 transition-colors">
              {tailorImages.length === 0 ? (
                <>
                  <ImageIcon size={18} className="text-[#D4AF37]" />
                  <p className="text-sm text-white/60">Glissez vos références ou cliquez pour choisir</p>
                </>
              ) : (
                <div className="grid grid-cols-2 gap-2 w-full">
                  {tailorImages.map((img, idx) => (
                    <div key={idx} className="relative aspect-square rounded-lg overflow-hidden border border-[#D4AF37]/20 bg-neutral-900">
                      <Image src={img} alt={`ref-${idx}`} fill className="object-cover" />
                    </div>
                  ))}
                </div>
              )}
          ...
          <div className="flex gap-2 flex-wrap">
            {['silhouette', 'motif', 'palette'].map((tag) => (
              <span
                key={tag}
                className="px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-[0.18em] bg-white/5 border border-white/10 text-white/60"
              >
                {tag}
              </span>
            ))}
          ...
          <motion.button
            ...
            className="w-full bg-white/5 border border-[#D4AF37]/30 text-[#D4AF37] py-3 rounded-xl font-black uppercase tracking-[0.2em] text-[11px] hover:bg-[#D4AF37]/10 transition-all"
          >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {inspiResults.map((ins) => (
              <article key={ins.id} className="bg-white/[0.03] border border-white/10 rounded-xl overflow-hidden">
                <div className="relative aspect-[3/4] bg-neutral-900">
                  <Image src={ins.image} alt={ins.title} fill className="object-cover" />
                  <div className="absolute inset-0 bg-gradient-to-t from-[#0A0A0A]/55 to-transparent" />
                  <div className="absolute bottom-2 left-2 flex items-center gap-1 text-[10px] uppercase tracking-[0.2em] text-[#D4AF37] font-black">
                    <ImageIcon size={12} />
                    IA
                  </div>
                </div>
                <div className="px-3 py-3 space-y-1">
                  <p className="text-sm font-serif text-[#D4AF37] leading-tight line-clamp-2">{ins.title}</p>
                  <p className="text-[11px] text-white/60 line-clamp-2">{ins.style}</p>
                </div>
              </article>
            ))}
          </div>
